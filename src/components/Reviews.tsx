    ...
    return (
        <div ref={columnRef} className={cn("animate-marquee space-y-8 py-4", className)} style={{ '--marquee-duration': duration } as React.CSSProperties}>
            {reviews.concat(reviews).map((imgSrc,reviewIndex)=>(
                <Review key={reviewIndex} className={reviewClassName?.(reviewIndex % reviews.length)} imgSrc={imgSrc}></Review>
            ))}
        </div>
    )
}
...
function Review({imgSrc,className,...props}:ReviewProps){
    ...
    return (
        <div className={cn(' rounded-[2.25rem] bg-white p-6 opacity-0 animate-fade-in shadow-xl shadow-slate-900/5',className)} style={{animationDelay}} {...props}>
            <Phone imgSrc={imgSrc}></Phone>
        </div>
    )
}
function ReviewGrid() {
    ...
    const [column1, column2] = splitArray<string>(PHONES, 3)
    const column3 = splitArray<string>(column2, 2)
    return (
        <div ref={containerRef} className='relative -mx-4 mt-16 grid h-[49rem] max-h-[150vh] grid-cols-1 items-start gap-8 overflow-hidden px-4 sm:mt-20 md:grid-cols-2 lg:grid-cols-3'>
            {isInView &&
                <>
                    ...
                    })} msPerPixel={10} ></ReviewColumn>
                    <ReviewColumn reviews={[...column2,...column3[1]]} className='hidden md:block' reviewClassName={(reviewIndex)=> reviewIndex >=column2.length?"lg:hidden":""} msPerPixel={15} ></ReviewColumn>
                    <ReviewColumn reviews={column3.flat()} className='hidden lg:block'  msPerPixel={10} ></ReviewColumn>
                </>
            }
...
function Reviews() {
    return (
        <MaxWidthWrapper className='relative max-w-5xl'>
            <img aria-hidden="true" src='/what-people-are-buying.png' className='absolute select-none hidden xl:block -left-32 top-1/3'></img>
